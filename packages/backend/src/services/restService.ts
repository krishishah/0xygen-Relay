...
export class RestService {
    ...
    constructor(
        ...
        private zeroExClient: ZeroExClient
    ) {
        this.init();
    }   
    ...
    public async postOrder(order: SignedOrder): Promise<void> {
        ...
        this.orderRepository
            .addOrUpdateOrder(order, orderHashHex)
            .then((_: SignedOrder) => {
            ...
            .catch(err => {
                ...
                this.orderRepository
                    .removeSignedOrder(order, orderHash)
    ...
    private async watchOrderbook(): Promise<void> {
        this.orderRepository
            .getAllSignedOrders()
            .then((orders: SignedOrder[]) => {
                orders.map(order => {
                    try {
                        this.watchOrder(order);
                    } catch ( e ) {
                        this.unwatchOrder(order);
                        const orderHash = ZeroEx.getOrderHashHex(order);
                        this.orderRepository.removeSignedOrder(order, orderHash);
    ...
    private onOrderWatcherEvent(err: Error | null, orderState?: OrderState) {
        if (orderState && orderState.isValid) {
            ...
            this.orderRepository
                ...
                .then(signedOrder => {
                    signedOrder.makerTokenAmount = orderRelevantState.remainingFillableMakerTokenAmount;
                    signedOrder.takerTokenAmount = orderRelevantState.remainingFillableTakerTokenAmount;
                    return signedOrder;
                })
                .then(newSignedOrder => {
                    this.orderRepository.addOrUpdateOrder(newSignedOrder, orderState.orderHash);
                    
                    const orderEvent: OrderEvent<OrderUpdated> = {
                        type: ORDER_UPDATED,
                        payload: {
                            order: newSignedOrder,
                            orderState: orderRelevantState,
                            makerTokenAddress: newSignedOrder.makerTokenAddress,
                            takerTokenAddress: newSignedOrder.takerTokenAddress
                        }
                    };
            ...
            this.orderRepository
            .getSignedOrder(state.orderHash)
            .then(signedOrder => {
                signedOrder.makerTokenAmount = new BigNumber(0);
                signedOrder.takerTokenAmount = new BigNumber(0);
                return signedOrder;
            })
            .then(signedOrder => {                    
                this.orderRepository.removeSignedOrder(signedOrder, state.orderHash);
    ...
    private init() {
        this.orderStateWatcher = ZeroExClient.zeroEx.createOrderStateWatcher(orderStateWatcherConfig);
        this.watchOrderbook()
            .then((value: void) => {
